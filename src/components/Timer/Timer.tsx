...
const PHASE_TABS = [
  { phase: "FOCUS", label: "Focus", color: "bg-focus text-white" },
...
const Timer = () => {
  ...
  return (
    <div className="pomodoro-timer flex flex-col items-center gap-6">
      <div aria-hidden="true" className="flex gap-2 rounded-full bg-white/70 p-1.5 shadow-sm shadow-ink/5">
        {PHASE_TABS.map((tab) => (
          <span
            key={tab.phase}
            className={cn(
              "rounded-full px-3.5 py-1.5 font-heading text-xs font-extrabold",
